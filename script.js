...
// ===== FIREBASE + WHATSAPP ONAY SİSTEMLİ =====
...
// NOT: BARBER_PHONE ve Firebase config artık config.js'te tanımlı
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    loadAppointments().then(() => {
        ...
        console.error('Veri yüklenemedi, boş liste ile devam ediliyor');
        if (document.getElementById('appointmentForm')) {
            initCustomerPage();
        }
    });
    
    // GERÇEK ZAMANLI DİNLEYİCİ - Firebase'de değişiklik olduğunda otomatik güncelle
    appointmentsRef.on('value', (snapshot) => {
        const data = snapshot.val();
        appointments = data ? Object.values(data) : [];
        console.log('🔄 Firebase güncellemesi alındı:', appointments.length, 'randevu');
        
        // Eğer müşteri sayfasındaysak saatleri güncelle
        if (document.getElementById('appointmentForm')) {
        ...
        // Eğer admin panelindeyse listeyi güncelle
...
// ===== RANDEVULARI KAYDET (FIREBASE) =====
function saveAppointments() {
    // Tüm randevuları Firebase'e kaydet
    appointmentsRef.set(appointments)
        .then(() => {
            console.log('✅ Randevular Firebase\'e kaydedildi');
        })
        .catch((error) => {
    ...
    return new Promise((resolve, reject) => {
        appointmentsRef.once('value')
            .then((snapshot) => {
                const data = snapshot.val();
                appointments = data ? Object.values(data) : [];
                console.log('✅ Firebase\'den', appointments.length, 'randevu yüklendi');
